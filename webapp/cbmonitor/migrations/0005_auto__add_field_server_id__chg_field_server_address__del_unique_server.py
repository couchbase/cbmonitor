...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'Server', fields ['address']
        db.delete_unique('cbmonitor_server', ['address'])

        # Adding field 'Server.id'
        db.add_column('cbmonitor_server', 'id',
                      self.gf('django.db.models.fields.AutoField')(default=1, primary_key=True),
                      keep_default=False)

        # Changing field 'Server.address'
        db.alter_column('cbmonitor_server', 'address', self.gf('django.db.models.fields.CharField')(max_length=80))
        # Adding unique constraint on 'Server', fields ['cluster', 'address']
        db.create_unique('cbmonitor_server', ['cluster_id', 'address'])

    def backwards(self, orm):
        # Removing unique constraint on 'Server', fields ['cluster', 'address']
        db.delete_unique('cbmonitor_server', ['cluster_id', 'address'])

        # Deleting field 'Server.id'
        db.delete_column('cbmonitor_server', 'id')

        # Changing field 'Server.address'
        db.alter_column('cbmonitor_server', 'address', self.gf('django.db.models.fields.CharField')(max_length=80, primary_key=True))
        # Adding unique constraint on 'Server', fields ['address']
        db.create_unique('cbmonitor_server', ['address'])
    ...
    models = {
        ...
        'cbmonitor.server': {
            'Meta': {'unique_together': "(['address', 'cluster'],)", 'object_name': 'Server'},
